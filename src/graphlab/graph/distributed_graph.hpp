...
#include <graphlab/graph/ingress/distributed_com_ingress.hpp>
...
namespace graphlab {
  ...
  public:
    ...
    friend class distributed_dc_ingress<VertexData, EdgeData>;
...
#pragma omp parallel for
  ...
  private:
    ...
    void set_ingress_method(const std::string& method,
      ...
      // } else if (method == "dc") {
      //   if (rpc.procid() == 0) logstream(LOG_EMPH) << "Use dc ingress" << std::endl;
      //   ASSERT_GT(nedges, 0); ASSERT_GT(nverts, 0);
      //   ingress_ptr = new distributed_dc_ingress<VertexData, EdgeData>(rpc.dc(), *this, threshold, nedges, nverts, interval);
      } else {
