...
#ifndef DHE_CUDA_IVERTEX_PROGRAM_HPP
#define DHE_CUDA_IVERTEX_PROGRAM_HPP
...
namespace graphlab {
  ...
  /**
   * \brief The cuda_ivertex_program class defines the vertex program
   ...
   * \endcode
   *
   * All user define vertex programs must extend the cuda_ivertex_program
   * interface and implement the cuda_ivertex_program::apply function.
   * Most vertex programs will also implement the
   * cuda_ivertex_program::gather and cuda_ivertex_program::scatter functions.
   *
   * The state of a vertex program *does not* persist between
   * invocations of \ref cuda_ivertex_program::init.  Moreover prior to
   ...
   * template arguments to cuda_ivertex_program interface.
   * 
   ...
   *
   * The message is received in the \ref cuda_ivertex_program::init
   * function.  The single message passed into 
   * \ref cuda_ivertex_program::init represents the sum of all messages 
   ...
   *   public cuda_ivertex_program<graph_type, gather_type, pagerank_message>,
  ...
  public:
     ...
     * \ref cuda_ivertex_program::gather_edges in parallel and returns the 
    ...
    //typedef struct cuda_graph_data_type<double,double,double> graph_data_type;

    // virtual __device__ float cuda_gather(size_t vertex_id, graph_data_type* graph_data)
    // {
    //   return 0.0;
    // }
    ...
    // virtual __device__ void cuda_apply(size_t vertex_id, graph_data_type* graph_data)
    // {
