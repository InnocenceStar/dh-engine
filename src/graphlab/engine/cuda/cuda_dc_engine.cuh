...
#ifdef GPU
#ifndef DC_ENGINE_HPP
#define DC_ENGINE_HPP
...
namespace graphlab {
   ...
   * \ref graphlab::dh_engine::signal or
   * \ref graphlab::dh_engine::signal_all.  In either case all
   * machines should invoke signal or signal all at the same time.  Finally,
   * computation is initiated by calling the
   * \ref graphlab::dh_engine::start function.
   *
   * ### Example Usage
   *
   ...
   *   graphlab::dh_engine<pagerank_vprog> engine(dc, graph, clopts);
   *   engine.signal_all();
   *   engine.start();
  ...
  private:
     ...
     */
    typedef context<dh_engine> context_type;
    friend class context<dh_engine>;
     ...
     * synchronous engine.
     */
    dc_dist_object< dh_engine<VertexProgram> > rmi;
    ...
    */
     ...
     * \ref graphlab::dh_engine::gather_accum
     * and \ref graphlab::dh_engine::messages.
     */
     ...
     * \ref graphlab::dh_engine::gather_accum and therefore is
    ...
    // de gpu done
    enum GPU_STATUS gpu_status = GPU_EXCUTE_GATHER_READY;
  ...
  public:
     ...
     */
    dh_engine(distributed_control& dc, graph_type& graph,
  ...
  private:
     ...
     *
     * \code
     * void dh_engine::member_fun(size_t threadid);
     * \endcode
     *
...
#define TEST_CUDA
    ...
    void run_synchronous(MemberFunction member_fun) {
      ...
      for(size_t i = 0; i < ncpus; ++i) {
        ...
        threads.launch(boost::bind(
              &dh_engine::thread_launch_wrapped_event_counter,
              this,
              invoke), affinity);
      }
#ifndef TEST_CUDA
    ...
    void execute_gathers_cuda(const size_t thread_id);

    std::vector<lvid_type> gpu_vertex_id_arr;

    typedef struct cuda_graph_data_type<vertex_data_type,edge_data_type,gather_type> cuda_graph_type;
    
    cuda_graph_type* cuda_graph_data;

    int  cuda_data_init_();

    void cuda_excute_gather_();


    void cuda_excute_apply_(const size_t thread_id);
    ...
    void execute_applys_cuda(const size_t thread_id);
    ...
    void print_hello(const size_t thread_id){
      using namespace std;
    ...
    size_t get_cpu_task(fixed_dense_bitset<8 * sizeof(size_t)>& local_bitset);
   ...
   */
  template<typename VertexProgram>
  dh_engine<VertexProgram>::
  dh_engine(distributed_control &dc,
                     graph_type& graph,
  ...
  template<typename VertexProgram>
  void dh_engine<VertexProgram>:: init() {
    resize();
    // Clear up
    force_abort = false;
    iteration_counter = 0;
    ...
    gpu_num = cuda_env_init();
    cuda_graph_data = (cuda_graph_type*)malloc(sizeof(cuda_graph_type));
    memset(cuda_graph_data,0,sizeof(cuda_graph_type));
  }


  template<typename VertexProgram>
  void dh_engine<VertexProgram>:: resize() {
  ...
  template<typename VertexProgram>
  typename dh_engine<VertexProgram>::aggregator_type*
  dh_engine<VertexProgram>::get_aggregator() {
    return &aggregator;
  ...
  template<typename VertexProgram>
  void dh_engine<VertexProgram>::internal_stop() {
    for (size_t i = 0; i < rmi.numprocs(); ++i)
      rmi.remote_call(i, &dh_engine<VertexProgram>::rpc_stop);
  } // end of internal_stop

  template<typename VertexProgram>
  void dh_engine<VertexProgram>::rpc_stop() {
    force_abort = true;
  } // end of rpc_stop


  template<typename VertexProgram>
  void dh_engine<VertexProgram>::
  ...
  internal_signal_gvid(vertex_id_type gvid, const message_type& message) {
    ...
    if(proc == rmi.procid()) internal_signal_rpc(gvid, message);
                         ...
                         &dh_engine<VertexProgram>::internal_signal_rpc,
                         gvid, message);
  } 
...
#define TEST_CUDA
  ...
  dh_engine<VertexProgram>::start() {
    ...
    //   run_synchronous( &dh_engine::initialize_vertex_programs );
    // }
    aggregator.start();
    rmi.barrier();
    ...
    while(iteration_counter < max_iterations && !force_abort ) {
...
#ifdef TEST_CUDA
      std::cout<<"----------begin exchange messages --------------------------"<<std::endl;
      shared_lvid_counter = 0;
      run_synchronous( &dh_engine::exchange_messages );
      fiber_control::affinity_type affinity;
      affinity.clear(); affinity.set_bit(ncpus-1);
      //boost::function<void(void)> invoke = boost::bind(&dh_engine::print_hello, this, ncpus-1);
      // threads.launch(boost::bind(
      //         &dh_engine::thread_launch_wrapped_event_counter,
      //         this,
      //         invoke), affinity);
      ...
      std::cout<<"----------end exchange messages --------------------------"<<std::endl;
#else
      run_synchronous( &dh_engine::exchange_messages );
#endif
      ...
      bk_ti.start();
#ifdef TEST_CUDA
      std::cout<<"----------begin receive messages --------------------------"<<std::endl;
      shared_lvid_counter = 0;
      run_synchronous( &dh_engine::receive_messages );
      fiber_control::affinity_type recv_affinity;
      recv_affinity.clear(); recv_affinity.set_bit(ncpus-1);
      // boost::function<void(void)> recv_invoke = boost::bind(&dh_engine::print_hello, this, ncpus-1);
      // threads.launch(boost::bind(
      //       &dh_engine::thread_launch_wrapped_event_counter,
      //       this,
      //       recv_invoke), recv_affinity);
      threads.join();
      rmi.barrier();
      if (ncpus <= 1) {
        DECREMENT_EVENT(EVENT_ACTIVE_CPUS, 1);
      }
      std::cout<<"----------end receive messages --------------------------"<<std::endl;
#else
      run_synchronous( &dh_engine::receive_messages );
#endif
      if (sched_allv) {
      ...
      bk_ti.start();
#ifdef TEST_CUDA
      std::cout<<"----------begin execute gathers --------------------------"<<std::endl;
      shared_lvid_counter = 0;
      //run_synchronous( &dh_engine::print_hello );
      run_synchronous( &dh_engine::execute_gathers );
      fiber_control::affinity_type gather_affinity;
      gather_affinity.clear(); gather_affinity.set_bit(ncpus-1);
      boost::function<void(void)> gather_invoke = boost::bind(&dh_engine::execute_gathers_cuda, this, ncpus-1);
      threads.launch(boost::bind(
            &dh_engine::thread_launch_wrapped_event_counter,
            this,
            gather_invoke), gather_affinity);
      //run_synchronous( &dh_engine::execute_gathers_cuda );
      threads.join();
      rmi.barrier();
      if (ncpus <= 1) {
        DECREMENT_EVENT(EVENT_ACTIVE_CPUS, 1);
      }
      std::cout<<"----------end execute gathers --------------------------"<<std::endl;
#else
      run_synchronous( &dh_engine::execute_gathers );
#endif
      ...
      bk_ti.start();
#ifdef TEST_CUDA
      std::cout<<"----------begin execute apply --------------------------"<<std::endl;
      shared_lvid_counter = 0;
      run_synchronous( &dh_engine::execute_applys );
      fiber_control::affinity_type apply_affinity;
      apply_affinity.clear(); apply_affinity.set_bit(ncpus-1);
      boost::function<void(void)> apply_invoke = boost::bind(&dh_engine::print_hello, this, ncpus-1);
      threads.launch(boost::bind(
            &dh_engine::thread_launch_wrapped_event_counter,
            this,
            apply_invoke), apply_affinity);
      threads.join();
      rmi.barrier();
      if (ncpus <= 1) {
        DECREMENT_EVENT(EVENT_ACTIVE_CPUS, 1);
      }
      std::cout<<"----------end execute apply --------------------------"<<std::endl;
#else
      run_synchronous( &dh_engine::execute_applys );
#endif
      ...
      bk_ti.start();
#ifdef TEST_CUDA
      std::cout<<"----------begin execute scatter --------------------------"<<std::endl;
      shared_lvid_counter = 0;
      run_synchronous( &dh_engine::execute_scatters );
      fiber_control::affinity_type scatter_affinity;
      scatter_affinity.clear(); scatter_affinity.set_bit(ncpus-1);
      // boost::function<void(void)> scatter_invoke = boost::bind(&dh_engine::print_hello, this, ncpus-1);
      // threads.launch(boost::bind(
      //       &dh_engine::thread_launch_wrapped_event_counter,
      //       this,
      //       scatter_invoke), scatter_affinity);
      threads.join();
      rmi.barrier();
      if (ncpus <= 1) {
        DECREMENT_EVENT(EVENT_ACTIVE_CPUS, 1);
      }
      std::cout<<"----------end execute scatter --------------------------"<<std::endl;
#else
      run_synchronous( &dh_engine::execute_scatters );
#endif
  ...
  template<typename VertexProgram>
  size_t  dh_engine<VertexProgram>::get_cpu_task(fixed_dense_bitset<8 * sizeof(size_t)>& local_bitset)
  {
    lvid_type lvid_block_start = shared_lvid_counter.inc_ret_last(8 * sizeof(size_t));
    if (lvid_block_start >= graph.num_local_vertices()) return -1;

    // get the bit field from has_message
    size_t lvid_bit_block = active_minorstep.containing_word(lvid_block_start);
    while (lvid_bit_block == 0) 
    {
      lvid_block_start = shared_lvid_counter.inc_ret_last(8 * sizeof(size_t));
      if (lvid_block_start >= graph.num_local_vertices()) return -1;

      // get the bit field from has_message
      lvid_bit_block = active_minorstep.containing_word(lvid_block_start);
    }
    // initialize a word sized bitfield
    local_bitset.clear();
    local_bitset.initialize_from_mem(&lvid_bit_block, sizeof(size_t));
    return lvid_block_start;
  }

  /**
   * \brief need to determine if the return value is empty
   * @return 0:have vertex to process;-1:out of the range of vertex nums
  */
  template<typename VertexProgram>
  int dh_engine<VertexProgram>::
  get_gpu_task()
  {
    ...
    lvid_type lvid_block_start = shared_lvid_counter.inc_ret_last(8 * sizeof(size_t));
    std::cout<<"gpu task local vertex id start:"<<lvid_block_start<<std::endl;
    if (lvid_block_start >= graph.num_local_vertices()) return -1;
    ...
    local_bitset.clear();
    local_bitset.initialize_from_mem(&lvid_bit_block, sizeof(size_t));
    // 通过重载begin，end和迭代器，使得foreach遍历时，lvid_block_offset是下一个bit是1的索引
    foreach(size_t lvid_block_offset, local_bitset) {
      lvid_type lvid = lvid_block_start + lvid_block_offset;
      std::cout<<"gpu task local vertex id :"<<lvid<<std::endl;
        if (lvid >= graph.num_local_vertices()) return -1;
        gpu_vertex_id_arr.push_back(lvid);
    }
    std::cout<<"gpu task local vertex id nums:"<<lvid_bit_block<<std::endl;
    return 0;
  }
  ...
  cuda_data_init_()
  {
    // get gpu task vertrex id as list
    int res = get_gpu_task();
    // complete gpu data struct init
    cuda_data_init(&gpu_vertex_id_arr, graph, cuda_graph_data);
    return res;
  }
  ...
  // cuda_excute_gather_()
  // {
  //   gpu_status = GPU_EXCUTE_GATHER_READY;
  //   int res = cuda_data_init_();
  //   // start to compute
  //   // cuda_excute_gather(cuda_graph_data,vertex_programs[0]); 
  //   cuda_excute_gather(cuda_graph_data); 
  //   // data clear
  //   gpu_status = GPU_EXCUTE_GATHER_DONE;
  // }

  template<typename VertexProgram>
  void dh_engine<VertexProgram>::
  execute_gathers_cuda(const size_t thread_id) {
    context_type context(*this, graph);
    ...
    while (1) {
      ...
      int res = cuda_data_init_();
      std::cout<<"cuda gpu task ver return num :"<<res<<std::endl;
      if(res == -1) {
        break;
      }
      cuda_excute_gather(cuda_graph_data); 
      // gpu sync---------------------------------------------------
      for(int i = 0;i < cuda_graph_data->vertex_list_len;++i){

        lvid_type lvid = (*(cuda_graph_data->vertex_id_list_incpu))[i];

        ++ngather_inc;
        if(caching_enabled) {
          gather_cache[lvid] = (*(cuda_graph_data->gather_nums_incpu))[i]; 
          has_cache.set_bit(lvid);
        }
        // If the accum contains a value for the local gather we put
        // that estimate in the gather exchange.
        sync_gather(lvid,(*(cuda_graph_data->gather_nums_incpu))[i], thread_id);
        if(!graph.l_is_master(lvid)) {
      ...
      cuda_graph_free(cuda_graph_data);
      free(cuda_graph_data);
    }// end of while
  ...
  execute_gathers(const size_t thread_id) {
    ...
    while (1) {
      ...
      // lvid_type lvid_block_start;
      // if((lvid_block_start = get_cpu_task(local_bitset))==-1){
      //   break;
      // }
      // 通过重载begin，end和迭代器，使得foreach遍历时，lvid_block_offset是下一个bit是1的索引
      foreach(size_t lvid_block_offset, local_bitset) {
        lvid_type lvid = lvid_block_start + lvid_block_offset;
        //std::cout<<"cpu task--"<<lvid<<std::endl;
        if (lvid >= graph.num_local_vertices()) break;
  ...
  template<typename VertexProgram>
  void dh_engine<VertexProgram>::
  cuda_excute_apply_(const size_t thread_id)
  {
    gpu_status = GPU_EXCUTE_APPLY_READY;
    // start to compute
    cuda_graph_data->gpu_status = gpu_status;
    cuda_graph_data->apply_gather_nums_ingpu = gather_accum.data();
    int res = cuda_data_init_();
    //cuda_excute_apply(cuda_graph_data,vertex_programs[0]); 
    cuda_excute_apply(cuda_graph_data); 
    // data clear
    gpu_status = GPU_EXCUTE_APPLY_DONE;
  }

  template<typename VertexProgram>
  void dh_engine<VertexProgram>::
  execute_applys_cuda(const size_t thread_id) {
    context_type context(*this, graph);
    const size_t TRY_RECV_MOD = 1000;
    size_t vcount = 0;
    size_t napply_inc = 0;
    ...
    while (1) {
      std::cout<< "gpu num:" << gpu_num <<std::endl;
      int res = cuda_data_init_();
      std::cout<<"apply cuda gpu task ver return num :"<<res<<std::endl;

      if(res == -1) {
        break;
      }
      cuda_excute_gather(cuda_graph_data); 
      //gpu sync vertex data-------------------------------------------------------
      for(int i = 0;i < cuda_graph_data->vertex_list_len;++i){         
        // sync gpu graph data to cpu
        lvid_type lvid = (*(cuda_graph_data->vertex_id_list_incpu))[i];
        local_vertex_type vertex = graph.l_vertex(lvid);
        vertex.data() = (*(cuda_graph_data->apply_gather_nums_incpu))[i];
  ...
  execute_applys(const size_t thread_id) {
    ...
    while (1) {
      ...
      foreach(size_t lvid_block_offset, local_bitset) {
        lvid_type lvid = lvid_block_start + lvid_block_offset;
        //std::cout<<"cpu apply task--"<<lvid<<std::endl;
        if (lvid >= graph.num_local_vertices()) break;
