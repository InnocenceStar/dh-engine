...
#ifndef DHE_CUDA_OMNI_ENGINE_HPP
#define DHE_CUDA_OMNI_ENGINE_HPP
...
namespace graphlab {
   ...
   * constructor of the cuda_omni_engine.  Forexample:
   *
   * \code
   ...
   * // line processing)
   * // Create the engine
   * graphlab::cuda_omni_engine<pagerank_vprog> engine(dc, graph, opts, exec_model);
   * \endcode
   *
   ...
   *
   * \code
   * graphlab::cuda_omni_engine<pagerank_vprog> engine(dc, graph, opts, "synchronous");
   * \endcode
   *
...
*
  ...
  class cuda_omni_engine : public iengine<VertexProgram> {
  public:
...
#ifdef GPU
    /**
     * \brief the type of distributed isomerism engine
    */
    typedef dh_engine<VertexProgram> dh_engine_type;
#endif
  ...
  private:
     ...
     */
    cuda_omni_engine(const cuda_omni_engine& other ) { }
  ...
  public:
     ...
     */
    cuda_omni_engine(distributed_control& dc, graph_type& graph,
...
#ifdef GPU
      } else if(engine_type == "dhe" || engine_type == "dh_engine"){
        engine_ptr = new dh_engine_type(dc, graph, new_options);
#endif
      } else {
