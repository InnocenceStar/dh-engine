...
#ifndef GRAPHLAB_HETEROGENEOUS_ENGINE_HPP
#define GRAPHLAB_HETEROGENEOUS_ENGINE_HPP
...
namespace graphlab {
   ...
   * \ingroup engines
   *
   * \brief The heterogeneous engine executes all active vertex program
   ...
   * \ref graphlab::heterogeneous_engine::signal or
   * \ref graphlab::heterogeneous_engine::signal_all.  In either case all
   * machines should invoke signal or signal all at the same time.  Finally,
   * computation is initiated by calling the
   * \ref graphlab::heterogeneous_engine::start function.
   *
   * ### Example Usage
   *
   ...
   *   graphlab::heterogeneous_engine<pagerank_vprog> engine(dc, graph, clopts);
   *   engine.signal_all();
   *   engine.start();
   ...
   * \see graphlab::semi_heterogeneous_engine
   */
  template<typename VertexProgram>
  class heterogeneous_engine :
  ...
  private:
     ...
     */
    typedef context<heterogeneous_engine> context_type;
    friend class context<heterogeneous_engine>;
     ...
     * synchronous engine.
     */
    dc_dist_object< heterogeneous_engine<VertexProgram> > rmi;
    ...
    */
     ...
     * \ref graphlab::heterogeneous_engine::gather_accum
     * and \ref graphlab::heterogeneous_engine::messages.
     */
     ...
     * \ref graphlab::heterogeneous_engine::vlocks
     */
     ...
     * \ref graphlab::heterogeneous_engine::gather_accum and therefore is
  ...
  public:
     ...
     */
    heterogeneous_engine(distributed_control& dc, graph_type& graph,
  ...
  private:
     ...
     *
     * \code
     * void heterogeneous_engine::member_fun(size_t threadid);
     * \endcode
     *
    ...
    void run_synchronous(MemberFunction member_fun) {
      ...
      for(size_t i = 0; i < ncpus; ++i) {
        ...
        threads.launch(boost::bind(
              &heterogeneous_engine::thread_launch_wrapped_event_counter,
              this,
              invoke), affinity);
      }
   ...
   */
  template<typename VertexProgram>
  heterogeneous_engine<VertexProgram>::
  heterogeneous_engine(distributed_control &dc,
                     graph_type& graph,
  ...
  template<typename VertexProgram>
  void heterogeneous_engine<VertexProgram>:: init() {
    resize();
    // Clear up
    force_abort = false;
    iteration_counter = 0;
  ...
  template<typename VertexProgram>
  void heterogeneous_engine<VertexProgram>:: resize() {
  ...
  template<typename VertexProgram>
  typename heterogeneous_engine<VertexProgram>::aggregator_type*
  heterogeneous_engine<VertexProgram>::get_aggregator() {
    return &aggregator;
  ...
  template<typename VertexProgram>
  void heterogeneous_engine<VertexProgram>::internal_stop() {
    for (size_t i = 0; i < rmi.numprocs(); ++i)
      rmi.remote_call(i, &heterogeneous_engine<VertexProgram>::rpc_stop);
  } // end of internal_stop

  template<typename VertexProgram>
  void heterogeneous_engine<VertexProgram>::rpc_stop() {
    force_abort = true;
  } // end of rpc_stop


  template<typename VertexProgram>
  void heterogeneous_engine<VertexProgram>::
  ...
  internal_signal_gvid(vertex_id_type gvid, const message_type& message) {
    ...
    if(proc == rmi.procid()) internal_signal_rpc(gvid, message);
                         ...
                         &heterogeneous_engine<VertexProgram>::internal_signal_rpc,
                         gvid, message);
  } 
  ...
  template<typename VertexProgram>
  size_t heterogeneous_engine<VertexProgram>::
  num_updates() const { return completed_applys.value; }

  template<typename VertexProgram>
  float heterogeneous_engine<VertexProgram>::
  elapsed_seconds() const { return timer::approx_time_seconds() - start_time; }

  template<typename VertexProgram>
  int heterogeneous_engine<VertexProgram>::
  iteration() const { return iteration_counter; }



  template<typename VertexProgram>
  size_t heterogeneous_engine<VertexProgram>::total_memory_usage() const {
  ...
  heterogeneous_engine<VertexProgram>::start() {
    if (vlocks.size() != graph.num_local_vertices())
      resize();
    completed_applys = 0;
    rmi.barrier();
    ...
    //   run_synchronous( &heterogeneous_engine::initialize_vertex_programs );
    // }
    aggregator.start();
    rmi.barrier();
    ...
    graphlab::timer iter_time;
    std::vector<double> excute_iter_time;
    std::vector<int> activate_iter_num;
    // Program Main loop ====================================================
    ti.start();
    while(iteration_counter < max_iterations && !force_abort ) {
      ...
      bk_ti.start();
      run_synchronous( &heterogeneous_engine::exchange_messages );
      ...
      bk_ti.start();
      run_synchronous( &heterogeneous_engine::receive_messages );
      if (sched_allv) {
      ...
      //     activate_iter_num.push_back(total_active_vertices);
      // }
      if (rmi.procid() == 0){
          logstream(LOG_EMPH)
          << "\tActive vertices: " << total_active_vertices << std::endl;
      ...
      bk_ti.start();
      run_synchronous( &heterogeneous_engine::execute_gathers );
      ...
      bk_ti.start();
      run_synchronous( &heterogeneous_engine::execute_applys );
      ...
      bk_ti.start();
      run_synchronous( &heterogeneous_engine::execute_scatters );
      ...
      ++iteration_counter;
      if (rmi.procid() == 0) {
        logstream(LOG_INFO)<<"-------------------iter time:::"<<iter_time.current_time()<<std::endl;
        excute_iter_time.push_back(iter_time.current_time());
      }
    ...
    if (rmi.procid() == 0) {
      logstream(LOG_INFO) <<"activate vertex number------";
      for (size_t i = 0;i < activate_iter_num.size(); ++i) {
        logstream(LOG_INFO)<< activate_iter_num[i] << ",";
      }
      logstream(LOG_INFO) <<"excute_iter_time------";
      for (size_t i = 0;i < excute_iter_time.size(); ++i) {
        logstream(LOG_INFO)<< excute_iter_time[i] << ",";
      }
